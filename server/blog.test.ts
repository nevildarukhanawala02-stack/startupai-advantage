...
describe("Blog API", () => {
  ...
  it("should have valid header image URLs for all posts", async () => {
    ...
    for (const post of posts) {
      // CloudFront CDN images or local storage proxy images
      expect(post.headerImageUrl).toMatch(/^(https:\/\/d2xsxph8kpxj0f\.cloudfront\.net\/.+\.webp|\/images\/.+\.png)$/);
    }
  });
  ...
  it("should have 7 infographic posts in CEO Intel Infographic Series", async () => {
    ...
    for (const post of igPosts) {
      ...
      expect(post.headerImageUrl).toMatch(/^\/images\/.+\.png$/);
    }
  });
